...
function vmtDumbbellChart(vehLabel, data) {
  ...
  const chart = Plot.plot({
    ...
    marks: [
      ...
      // top where the two dots overlap. Target uses themeTargetAlt (green)
      // here instead of the usual themeTarget blue -- themeTarget would
      // blend into the "Model under target" end of themeDiffColor's
      // red/blue ring below.
      Plot.dot(data, {
        x: "TargetM", y: "County", fill: themeTargetAlt, r: 8,
        title: d => `${d.County} — ${d.VehLabel} — ${d.ClassLabel}\nTarget: ${d3.format(",.2f")(d.TargetM)}M VMT`
      }),
      Plot.dot(data, {
        x: "ModelM", y: "County", fill: "none", stroke: d => themeDiffColor(d.vmtDiffPct), strokeWidth: 3, r: 8,
...
Plot.plot({
  ...
  marks: [
    ...
    // top where the two dots overlap. Observed uses themeTargetAlt (green)
    // here instead of the usual themeTarget blue -- themeTarget would
    // blend into the "Model under target" end of themeDiffColor's
    // red/blue ring below.
    Plot.dot(regionTimeSummary, {
      x: "obsMean", y: "Period", fill: themeTargetAlt, r: 8,
      title: d => `${d.Period}\nObserved: ${d3.format(",.1f")(d.obsMean)} min`
    }),
    Plot.dot(regionTimeSummary, {
      x: "modMean", y: "Period", fill: "none", stroke: d => themeDiffColor(d.diffPct), strokeWidth: 3, r: 8,
