...
@fig-assign-vmt-dumbbell compares modeled and target daily VMT for Freeway and Non-Freeway roads across each county; @fig-assign-compare-interactive shows the same model-vs-target differences at the individual segment level. Both share the Vehicle Type and Display controls below; Focus Roadway Class additionally narrows the chart and dims non-matching segments on the map. The values for Box Elder and Weber counties are only the portions within the MPO planning area.
...
vmtGroupRows = vmtT.filter(d => d.ClassLevel === 'FTGROUP' && d.vehType === bVehType && d.calib_run === globalCalibRun && d.CO_FIPS !== 99)

// "All" sums Freeway + Non-Freeway into one row per county (not a facet per
// class) so the chart is a single dumbbell view rather than two panels.
dumbbellData = bFocusClass === 'All'
  ? Array.from(
      d3.rollup(
        vmtGroupRows,
        rows => {
      ...
      ([CO_FIPS, agg]) => ({
        ...agg,
        County: countyNames.get(CO_FIPS),
        ClassLabel: 'All Roadways',
        ModelM: agg.VMT_Mod / 1e6,
        TargetM: agg.VMT_Target / 1e6
      })
    )
  : vmtGroupRows
      .filter(d => d.FTCLASS === bFocusClass)
      .map(d => ({
        ...d,
...
//| fig-cap: Model vs Target VMT by Roadway Class and Geography
Plot.plot({
  width: 380,
  height: 280,
  marginLeft: 130,
  title: bFocusClass === 'All' ? 'All Roadways' : classLabels.get(bFocusClass),
  ...
  marks: [
    ...
    Plot.dot(dumbbellData, {
      x: "ModelM", y: "County", fill: themeModel, r: 6,
      title: d => `${d.County} — ${d.ClassLabel}\nModel: ${d3.format(",.2f")(d.ModelM)}M VMT`
    }),
    Plot.dot(dumbbellData, {
      x: "TargetM", y: "County", fill: themeTarget, r: 6,
...
// 2. MAP RENDERING (shares bVehType / bDiffType with the chart at left;
// bFocusClass additionally dims non-matching segments on this map only)
{
