...
@fig-assign-vmt-dumbbell always shows every county, as one chart per Vehicle Type (Total, Medium CV, Heavy CV) -- each independently scaled to its own VMT range, since Medium/Heavy CV volumes are much smaller than Total -- with Roadway Class (All Roadways, Freeway, Non-Freeway) faceted down the rows within each. Only its diff labels follow the Display control below. @fig-assign-compare-interactive, below the chart, shows the same model-vs-target differences at the individual segment level and responds to all three controls: Vehicle Type, Display, and Focus Roadway Class. The values for Box Elder and Weber counties are only the portions within the MPO planning area.
...
// still selectable via Vehicle Type above for the map/table).
...
// The chart is a fixed 3x3 facet grid (always every Vehicle Type x Roadway
// Class combination) rather than reacting to Vehicle Type / Focus Roadway
// Class -- those two controls now only affect the map and table below.
// Freeway/Non-Freeway rows come straight from the data; "All Roadways" is
// computed here by summing the two per County x Vehicle Type.
dumbbellData = {
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.text(data, {
        ...
        text: d => bDiffType === 'Pct' ? d3.format("+.1%")(d.vmtDiffPct) : d3.format("+,.2f")(d.vmtDiff / 1e6) + "M",
...
// 2. MAP RENDERING (shares bVehType / bDiffType with the chart above, which
// no longer reacts to either -- bFocusClass dims non-matching segments here)
{
...
The tables below are a full reference view -- one per geography, always listing Freeway, Non-Freeway, and an All Roadways subtotal for the selected Vehicle Type, independent of the Focus Roadway Class control above.
...
// Non-Freeway rows plus an "All Roadways" subtotal row.
