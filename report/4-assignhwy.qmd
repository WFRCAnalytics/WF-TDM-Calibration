...
@fig-assign-vmt-dumbbell compares modeled and target daily VMT for Freeway and Non-Freeway roads across the region and each county; @fig-assign-compare-interactive shows the same model-vs-target differences at the individual segment level. Both share the Vehicle Type and Display controls below; Focus Geography and Focus Roadway Class narrow the map to a single county and/or roadway class. The values for Box Elder and Weber counties are only the portions within the MPO planning area.
...
viewof bFocusCounty = Inputs.select(
  new Map([
    ["Region", 99],
  ...
  {value: 99, label: "Focus Geography:"}
)
...
dumbbellData = vmtT
  .filter(d => d.ClassLevel === 'FTGROUP' && d.vehType === bVehType && d.calib_run === globalCalibRun && d.CO_FIPS !== 99 && (bFocusClass === 'All' || d.FTCLASS === bFocusClass))
  .map(d => ({
    ...d,
...
Plot.plot({
  width: 380,
  height: bFocusClass === 'All' ? 520 : 280,
  marginLeft: 130,
  facet: {data: dumbbellData, y: "ClassLabel"},
  fy: {domain: bFocusClass === 'All' ? ["Freeway", "Non-Freeway"] : [classLabels.get(bFocusClass)]},
...
// bFocusCounty / bFocusClass additionally pan and dim this map only)
{
  ...
  if (runFeatures.length) {
    LM.geoJSON({type: "FeatureCollection", features: runFeatures}, {
      style: (feature) => {
        ...
        const classMatch = bFocusClass === 'All' || bFocusClass === ftgroup;
        const countyMatch = bFocusCounty === 99 || Number(p.CO_FIPS) === bFocusCounty;
        const focused = classMatch && countyMatch;
    ...
    // Focus Geography pans/zooms to that county's segments; "Region" keeps the default full-region view.
    if (bFocusCounty !== 99) {
      const focusFeatures = runFeatures.filter(f => Number(f.properties.CO_FIPS) === bFocusCounty);
      if (focusFeatures.length) {
        map.fitBounds(LM.geoJSON({type: "FeatureCollection", features: focusFeatures}).getBounds(), {padding: [30, 30]});
      }
    }
  }
...
The table below is a full reference view -- it always lists every county and roadway class for the selected Vehicle Type, independent of the Focus controls above.
...
tableRows = vmtT
  .filter(d => d.ClassLevel === 'FTGROUP' && d.vehType === bVehType && d.calib_run === globalCalibRun)
  .map(d => ({
    County: countyNames.get(d.CO_FIPS),
    ClassLabel: classLabels.get(d.FTCLASS) ?? d.FTCLASS,
    VMT_Mod: d.VMT_Mod,
    ...
    vmtDiff: d.vmtDiff,
    vmtDiffPct: d.vmtDiffPct
  }))
  .sort((a, b) => countyOrder.indexOf(a.County) - countyOrder.indexOf(b.County) || d3.ascending(a.ClassLabel, b.ClassLabel))
```
...
<table style="width:100%; border-collapse:collapse;">
  <thead>
    <tr>
      ${["County", "Roadway Class", "Model", "Target", "Difference", "Percent Difference"].map((d, i) => html`<th style='text-align: ${i < 2 ? "left" : "right"}; padding: 6px 10px; border-bottom: 2px solid #999;'>${d}</th>`)}
    </tr>
  </thead>
  <tbody>
    ${tableRows.map(row => html`<tr style='border-bottom: 1px solid lightgrey;'>
      <td style='padding: 6px 10px;'>${row.County}</td>
      <td style='padding: 6px 10px;'>${row.ClassLabel}</td>
      <td style='padding: 6px 10px; text-align:right;'>${fmtVmt(row.VMT_Mod)}</td>
      <td style='padding: 6px 10px; text-align:right;'>${fmtVmt(row.VMT_Target)}</td>
      <td style='padding: 6px 10px; text-align:right; color:${Math.abs(row.vmtDiffPct) > 0.1 ? themeDanger : "inherit"};'>${fmtVmt(row.vmtDiff)}</td>
      <td style='padding: 6px 10px; text-align:right; color:${Math.abs(row.vmtDiffPct) > 0.1 ? themeDanger : "inherit"};'>${fmtPct(row.vmtDiffPct)}</td>
    </tr>`)}
  </tbody>
</table>`
```
