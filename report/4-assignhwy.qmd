...
Plot.plot({
  ...
  marks: [
    ...
    Plot.linearRegressionY(dash_data.scatter, {
      x: "Obs_K", y: "Mod_K", stroke: themeModel,
...
Plot.plot({
  ...
  color: { domain: ["Observed", "Modeled"], range: [themeObserved, themeModel], legend: true, label: "Data Source" },
...
map_display = {
  ...
  const stationLayer = L.geoJSON(filtered_sites, {
    ...
    onEachFeature: (feature, layer) => {
      ...
      if (stats) {
        const diffColor = stats.Pct_Error > 0 ? themeDanger : themeObserved;
...
df_wide = mod_obs_df.pivot(
    index=["SEGID", "calib_run"], columns="vehType", values=["AWDT_Mod", "AWDT_Obs"]
)
...
sum_class = all_df.groupby(['CO_FIPS','FTCLASS','vehType','calib_run'], as_index=False).agg(agg_functions_all)
sum_class['ClassLevel'] = 'FTCLASS'

# 2. Summarize by FTGROUP
sum_group = all_df.groupby(['CO_FIPS','FTGROUP','vehType','calib_run'], as_index=False).agg(agg_functions_all)
...
sum_all_df = sum_all_df.drop_duplicates(subset=['CO_FIPS', 'FTCLASS', 'vehType', 'ClassLevel', 'calib_run'])
...
allveh_vol = sum_all_df[['CO_FIPS','FTCLASS','vehType','ClassLevel','calib_run','numSegs','AWDT_Mod','AWDT_Obs','volDiff','volDiffPct','volRmse','volRmsePct']]
allveh_vmt = sum_all_df[['CO_FIPS','FTCLASS','vehType','ClassLevel','calib_run','numSegs','VMT_Mod','VMT_Obs','vmtDiff','vmtDiffPct']]
```

```{python}
# create summary comparison tables for ojs - percent difference table

allveh_pct = sum_all_df[['CO_FIPS','FTCLASS','vehType','ClassLevel','calib_run','volDiffPct','vmtDiffPct']]
...
allveh_pct = allveh_pct.pivot(
    index=['FTCLASS','vehType','ClassLevel','calib_run','Variable'],
    columns='CO_FIPS',
    values='Value'
).reset_index()

allveh_pct = allveh_pct[['Variable', 'FTCLASS', 'vehType', 'ClassLevel', 'calib_run', 3, 57, 11, 35, 49, 99]] #99:Region
#allveh_pct
```
...
allveh_pct_exact = sum_all_df_copy[
    ["CO_FIPS", "FTCLASS", "vehType", "ClassLevel", "calib_run", "volDiffPct", "vmtDiffPct"]
]
...
allveh_pct_exact = allveh_pct_exact[
    ["Variable", "FTCLASS", "vehType", "ClassLevel", "calib_run", 3, 57, 11, 35, 49, 99]
]

allveh_pct_exact = allveh_pct_exact.melt(
    id_vars=["FTCLASS", "vehType", "ClassLevel", "calib_run", "Variable"],
...
allveh_abs_exact = sum_all_df_copy[['CO_FIPS', 'FTCLASS', 'ClassLevel', 'calib_run', 'AWDT_Mod', 'AWDT_Obs', 'VMT_Mod', 'VMT_Obs']]

allveh_abs_exact = allveh_abs_exact.melt(
    id_vars = ['CO_FIPS','FTCLASS','ClassLevel','calib_run'],
...
allvehplot = all_df[['SEGID','CO_FIPS','FTCLASS','vehType','calib_run','AWDT_Mod','AWDT_Obs','volErrorPct']].copy()
...
Plot.plot({
  ...
  color: {
    ...
    range: [themeModel,  themeObserved, themeObservedLight]
  },
  ...
  marks: [
    ...
    plot_type === "Absolute" ? [
       ...
       Plot.areaX(plot_data, Plot.binY({ x1: bin => -bin.length, x2: "count" }, {
           y: "obs_val", fx: "CO_SEG", fill: themeObservedLight, fillOpacity: 0.7, curve: "basis", thresholds: 15
       })),

       // Observed Mean Dot
       Plot.dot(plot_data, {
           y: "mean_obs", fx: "CO_SEG", x: 0, fill: themeObserved, r: 4.5, symbol: "circle",
    ...
    [
       ...
       Plot.areaX(plot_data, Plot.binY({ x1: bin => -bin.length, x2: "count" }, {
           y: "diff_from_mean", fx: "CO_SEG", fill: themeObservedLight, fillOpacity: 0.7, curve: "basis", thresholds: 15
       })),

       // Observed Mean Anchor Dot
       Plot.dot(plot_data, {
           y: 0, fx: "CO_SEG", x: 0, fill: themeObserved, r: 4.5, symbol: "circle",
           // UNIFIED TOOLTIP
  ...
  const stationLayer = L.geoJSON(filtered_stations, {
    pointToLayer: (feature, latlng) => {
      ...
      if (stats) {
          ...
          else finalColor = themeObserved; // within tolerance
      }
