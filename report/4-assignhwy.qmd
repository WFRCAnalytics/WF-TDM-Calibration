...
// One chart per Vehicle Type, each auto-scaled to its own data's range --
// Total, Medium CV, and Heavy CV differ by roughly an order of magnitude,
// so sharing one x-axis (as a single faceted grid) squished the two CV
// columns down near zero. Roadway Class stays faceted as rows within each.
function vmtDumbbellChart(vehLabel, data) {
  return Plot.plot({
    ...
    marginRight: 60,
    title: vehLabel,
    facet: {data, y: "ClassLabel"},
    fy: {domain: ["All Roadways", "Freeway", "Non-Freeway"]},
...
html`<div>
  ${["Total", "Medium CV", "Heavy CV"].map(veh => vmtDumbbellChart(veh, dumbbellData.filter(d => d.VehLabel === veh)))}
</div>`
```
