...
```{python}
# create long format for the Model vs Target bar chart (chart 2b)
allveh_vmt_long = allveh_vmt.rename(columns={'VMT_Mod': 'Model', 'VMT_Target': 'Target'})
allveh_vmt_long = pd.melt(allveh_vmt_long,
                          id_vars=['CO_FIPS', 'FTCLASS', 'vehType', 'ClassLevel', 'calib_run'],
                          value_vars=['Model', 'Target'],
                          var_name='DataSource',
                          value_name='ViewValue')
allveh_vmt_long['ViewValue'] = allveh_vmt_long['ViewValue'] / 1000000

# long format for the Summary Comparison diverging bar chart (region/county)
...
The summary comparison shows region and county-wide differences between modeled and target *Vehicle-Miles Traveled (VMT)* by vehicle type. The values for Box Elder and Weber counties are only the portions within the MPO planning area. @fig-assign-summary, below, contains an interactive view of model vs target differences by roadway class and vehicle type.
...
vmtDiffLongT_filtered = vmtDiffLongT.filter(function(dataL) {
    return bSummaryFTCLASS == dataL.FTCLASS &&
           ...
           dataL.calib_run == globalCalibRun;
})
```
...
*(Note: the paragraphs below described the previous model-vs-observed-volume comparison, which has been replaced by the model-vs-target VMT comparison above. Replace this placeholder with fresh observations once the report has been re-rendered against the new chart.)*


### Detailed Comparison

The model vs target details in this section are presented as Vehicle-Miles Traveled (VMT) through the comparison of modeled VMT (summed across all segments in the model's segment summary) and calibration VMT targets, by facility type, for the region and each county. @fig-assign-validation allows for the interactive visual comparison of model and target values for the region and each county for all vehicles, cars, Medium CV, and Heavy CV.

- *Total VMT by Roadway Class*: For each county (or the region) and roadway class, all segments' modeled daily VMT is summed and compared against the calibration VMT target for that county/facility-type combination.
...
//| fig-cap: Model vs Target VMT Comparison
...
function renderVmtRow(row, isTotal=false) {
    ...
    return html`<tr style='${rowStyle}'>
        ${["FTCLASS", "VMT_Mod", "VMT_Target", "vmtDiff", "vmtDiffPct"].map((d, i) => {
            ...
            return html`<td style='text-align: ${i === 0 ? "left" : "right"}; padding: 5px; font-weight: ${isTotal ? 'bold' : 'normal'};'>${formattedValue}</td>`;
        })}
    </tr>`;
}
```
...
```{ojs}
// Body rows exclude any precomputed "All Roadways" row (only present for the
// FTCLASS grouping) -- the totals row below is derived instead, so it also
// shows up (correctly summed) for the FTGROUP grouping, which has no such row.
bodyRows = filtered_vmtData.filter(d => d.FTCLASS !== 'All Roadways')

totalRow = filtered_vmtData.find(d => d.FTCLASS === 'All Roadways') ?? (() => {
    const modSum = d3.sum(bodyRows, d => d.VMT_Mod);
    const targetSum = d3.sum(bodyRows, d => d.VMT_Target);
    return {
        FTCLASS: 'All Roadways',
        VMT_Mod: modSum,
...
```{ojs}
html`
<h4>Total Daily VMT by Roadway Class</h4>
<table>
    <thead>
    <tr>
        ${["Roadway Class", "Model", "Target", "Difference", "Percent Difference"].map((d, i) => {
            return html`<th style='text-align: ${i === 0 ? "left" : "right"}; padding: 5px; width: ${widthsVmt[i]};'>${d}</th>`;
        })}
    </tr>
    </thead>
    <tbody>
        ${bodyRows.map(row => renderVmtRow(row))}
    </tbody>
    <tfoot>
        ${renderVmtRow(totalRow, true)}
    </tfoot>
</table>`
```
...
<!-- ::: {layout-ncol=3}
![Segment-Level Model vs Target Volume Comparison -- All Vehicles](_pictures/vol-diff-Total.png){#fig-assign-compare-all}

![Segment-Level Model vs Target Volume Comparison -- Medium CV](_pictures/vol-diff-MD.png){#fig-assign-compare-md}

![Segment-Level Model vs Target Volume Comparison -- Heavy CV](_pictures/vol-diff-HV.png){#fig-assign-compare-hv}
::: -->
...
viewof mapType = Inputs.radio(
  [ "Absolute", "Percent" ],
  { value: "Absolute", label: "Difference Type:" }
)
```
...
// 3. MAP RENDERING
{
  ...
  legend.onAdd = function() {
    ...
    let labels = [`<strong>${mapType === "Percent" ? "% Diff" : "Diff"} (Mod - Tgt)</strong><br>`];
